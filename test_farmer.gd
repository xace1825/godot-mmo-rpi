...
func _physics_process(_delta):
	...
	match test_phase:
		...
		"wait_stock":
			if elapsed > 3.0:
				Network.ask_build(Vector2i(72, 64), PlanetGenerator.BuildingType.FARM)
				print("TEST: placed farm blueprint")
...
func _on_full_sync(data: Dictionary):
	...
	var food_before: int = resources.get("food", 0)
	if test_phase == "wait_food" and food_before > 500:
		_pass("farmers produced food: %d" % food_before)
		_finish()
